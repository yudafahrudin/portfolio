...
export const getCurrentDate = () => parseISO(format(new Date(), "yyyy-MM-dd"));

export const getCurrentYear = () => format(new Date(), "yyyy");

export const setYearFromNow = (year: number): Date =>
    subYears(getCurrentDate(), year);

export const setMonthFromNow = (month: number): Date =>
    subMonths(getCurrentDate(), month);

export const defaultDateFormat = (date: Date, formatPattern?: string) =>
